...
public class NurseSchedulingProblem {
    ...
    public static void main(String[] args) {
        ...
        BoolVar[][][][] timetable = new BoolVar[numHours][numCourses][numRooms][numTeachers];
        for (int h = 0; h < numHours; h++) {
            for (int c = 0; c < numCourses; c++) {
                for (int r = 0; r < numRooms; r++) {
                    for (int t = 0; t < numTeachers; t++) {
                        timetable[h][c][r][t] = model.newBoolVar("h" + h + "_c" + c + "_r" + r + "_t" + t);
                    }
                }
            }
        }
        ...
        if (status == CpSolverStatus.OPTIMAL || status == CpSolverStatus.FEASIBLE) {
            ...
            for (int h = 0; h < numHours; h++) {
                for (int c = 0; c < numCourses; c++) {
                    for (int r = 0; r < numRooms; r++) {
                        for (int t = 0; t < numTeachers; t++) {
                            if (solver.booleanValue(timetable[h][c][r][t])) {
                                System.out.println("Hour: " + h + ", Course: " + c + ", Room: " + r + ", Teacher: " + t);
                            }
                        }
                    }
                }
