...
public class TimetablePdfExportService {
    ...
    // Boshqa repositorylar kerak emas, chunki ism-familiyalar 'slotDetails' ichida saqlangan (Response obyekti sifatida).
    // Lekin agar siz Entitydan ID olib, qayta nomini qidirmoqchi bo'lsangiz, repositorylar qoladi.
    // Eng optimali: slotDetails ichidagi tayyor 'TeacherResponse', 'SubjectResponse' obyektlaridan ismini olish.
    ...
    public byte[] exportToPdf(UUID timetableId) throws IOException {
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            for (DayOfWeek day : days) {
                ...
                // Day Cell
                PdfPCell dayCell = new PdfPCell(new Phrase(toTitleCase(day.name()), FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10)));
                ...
                for (int period = 1; period <= maxPeriod; period++) {
                    // Period Cell
                    PdfPCell periodCell = new PdfPCell(new Phrase(String.valueOf(period), FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10)));
     ...
     * Agar guruhli dars bo'lsa (Split), ularni chiroyli qilib (/) bilan ajratib ko'rsatadi.
     */
