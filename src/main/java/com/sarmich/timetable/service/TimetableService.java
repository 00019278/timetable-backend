...
public class TimetableService {
  ...
  public void generateTimetable(GenerateReply reply) {
    try {
      ...
      Map<Integer, ClassResponse> classMap =
          Util.mapById(
              classRepository.findAllByOrgIdAndDeletedFalse(orgId).stream()
                  .map(ClassMapper.INSTANCE::toResponse)
                  .toList(),
              ClassResponse::id);
      Map<Integer, TeacherResponse> teacherMap =
          Util.mapById(
              teacherRepository.findAllByOrgIdAndDeletedFalse(orgId).stream()
                  .map(TeacherMapper.INSTANCE::toResponse)
                  .toList(),
              TeacherResponse::id);
      Map<Integer, SubjectResponse> subjectMap =
          Util.mapById(
              subjectRepository.findAllByOrgIdAndDeletedFalse(orgId).stream()
                  .map(SubjectMapper.INSTANCE::toResponse)
                  .toList(),
              SubjectResponse::id);
      Map<Integer, RoomResponse> roomMap =
          Util.mapById(
              roomRepository.findAllByOrgIdAndDeletedFalse(orgId).stream()
                  .map(RoomMapper.INSTANCE::toResponse)
                  .toList(),
              RoomResponse::id);
      // GroupMap ni to'ldirish - barcha sinflarning guruhlarini yuklaymiz
      List<Integer> classIds = classMap.keySet().stream().toList();
      Map<Integer, GroupResponse> groupMap =
          Util.mapById(
              groupRepository.findAllByClassIdInAndOrgIdAndDeletedFalse(classIds, orgId).stream()
                  .map(GroupMapper.INSTANCE::toResponse)
                  .toList(),
              GroupResponse::id);

      // 1. Solver uchun tayyorlash (Flat + SyncId)
      List<OrTLesson> orTLessons =
          solverDataPreparer.prepareLessons(
              lessons, classMap, teacherMap, subjectMap, roomMap, groupMap);
  ...
  private void saveData(
    ...
    Map<Integer, LessonResponse> lessonMapById =
        lessons.stream().collect(Collectors.toMap(LessonResponse::id, l -> l, (v1, v2) -> v1));
    ...
    for (TimetableSlotResponse slot : generate.getScheduledSlots()) {
      ...
      List<TimetableGroupDetail> details =
          slot.getDetails().stream()
              .map(
                  d -> {
                    d.setOriginalLessonData(lessonMapById.get(d.getLessonId()));
                    return d;
                  })
              .toList();
      entity.setSlotDetails(details);
      entitiesToSave.add(entity);
    }

    // 2. Unscheduled
    for (UnscheduledLesson un : generate.getUnscheduledLessons()) {
      TimetableDataEntity entity = new TimetableDataEntity();
      entity.setTimetableId(timetableId);
      entity.setClassId(un.getClassId());
      entity.setIsScheduled(false);
      entity.setVersion(version);
      entity.setUnscheduledData(un);
      entitiesToSave.add(entity);
    }

    timetableDataRepository.saveAll(entitiesToSave);

    // 3. Update Timetable Metadata
    TimetableEntity timetable = timetableRepository.findByIdAndDeletedFalse(timetableId);
    if (timetable != null) {
      ScheduleScore score = scoreCalculatorService.calculateScore(entitiesToSave, rules);
      timetable.setScore(score.getTotalPenaltyScore());

      // Jami darslar sonini hisoblash (Slotlar ichidagi detailar soni)
      int totalScheduledLessons =
          generate.getScheduledSlots().stream().mapToInt(s -> s.getDetails().size()).sum();

      timetable.setScheduled(totalScheduledLessons);
      timetable.setUnscheduled(generate.getUnscheduledLessons().size());
  ...
  public void optimize(UUID id, ApplySoftConstraint request) {
    ...
    List<LessonResponse> originalLessons =
        reconstructLessonsFromTimetableData(timetableData, company.id());

    // 2. Solver uchun Flat formatga o'tkazish (Split darslarni SyncId bilan
    // ajratish)
    ...
    Set<Integer> classIds =
        timetableData.stream().map(TimetableDataEntity::getClassId).collect(Collectors.toSet());

    Map<Integer, ClassResponse> classHintMap =
        Util.mapById(
    ...
    Map<Integer, TeacherResponse> teacherHintMap =
        Util.mapById(
            teacherRepository.findAllByOrgIdAndDeletedFalse(timetable.getOrgId()).stream()
                .map(TeacherMapper.INSTANCE::toResponse)
                .toList(),
            TeacherResponse::id);

    Map<Integer, SubjectResponse> subjectHintMap =
        Util.mapById(
            subjectRepository.findAllByOrgIdAndDeletedFalse(timetable.getOrgId()).stream()
                .map(SubjectMapper.INSTANCE::toResponse)
                .toList(),
            SubjectResponse::id);

    Map<Integer, RoomResponse> roomHintMap =
        Util.mapById(
            roomRepository.findAllByOrgIdAndDeletedFalse(timetable.getOrgId()).stream()
                .map(RoomMapper.INSTANCE::toResponse)
                .toList(),
            RoomResponse::id);

    // GroupMap ni to'ldirish
    List<Integer> classIdsForGroups = classHintMap.keySet().stream().toList();
    Map<Integer, GroupResponse> groupMap =
        Util.mapById(
            groupRepository
                .findAllByClassIdInAndOrgIdAndDeletedFalse(classIdsForGroups, timetable.getOrgId())
                .stream()
                .map(GroupMapper.INSTANCE::toResponse)
                .toList(),
            GroupResponse::id);
    ...
    // 2. Solver uchun Flat formatga o'tkazish (Split darslarni SyncId bilan
    // ajratish)
    // Note: classHintMap might not cover all classes if originalLessons spans more,
    // but usually covered.
    // Better to fetch ALL classes if possible, but for optimization context usually
    // limited scope is fine.
    // However, prepareLessons logic assumes access to all relevant classes.
    // classHintMap is built from timetableData, but originalLessons might have
    // more?
    // Actually originalLessons comes from reconstructLessonsFromTimetableData which
    // uses timetableData. So classHintMap is sufficient.
    List<OrTLesson> orTLessons =
        solverDataPreparer.prepareLessons(
            originalLessons, classHintMap, teacherHintMap, subjectHintMap, roomHintMap, groupMap);

    // 3. Oldingi jadvalni olish (Hint uchun)
    // MUHIM O'ZGARISH: SlotDetails (JSON List) ni yoyib, Response (Hint) ga
    // aylantiramiz
    ...
    for (TimetableDataEntity entity : timetableData) {
      ...
      if (Boolean.TRUE.equals(entity.getIsScheduled()) && entity.getSlotDetails() != null) {
        ...
        for (TimetableGroupDetail detail : entity.getSlotDetails()) {
          ...
          hint.setLessonId(
              detail.getLessonId()); // <-- Eng muhimi shu! WarmStartService shuni qidiradi

          if (detail.getTeacherId() != null)
            hint.setTeacher(teacherHintMap.get(detail.getTeacherId()));
          if (detail.getSubjectId() != null)
            hint.setSubject(subjectHintMap.get(detail.getSubjectId()));

          if (detail.getRoomId() != null)
            hint.setRoom(roomHintMap.get(detail.getRoomId())); // Aniq tanlangan xona
  ...
  private List<LessonResponse> reconstructLessonsFromTimetableData(
    ...
    for (LessonResponse oldLesson : uniqueLessons.values()) {
      ...
      TeacherResponse freshTeacher =
          (oldLesson.teacherId() != null) ? teacherMap.get(oldLesson.teacherId()) : null;
      ...
      if (oldLesson.groupDetails() != null) {
        for (GroupLessonDetailResponse oldDetail : oldLesson.groupDetails()) {
          ...
          GroupLessonDetailResponse freshDetail =
              new GroupLessonDetailResponse(
                  groupId, detailTeacherId, detailSubjectId, detailRoomIds);
          freshGroupDetails.add(freshDetail);
        }
      }

      // Yangi LessonResponse yaratish
      LessonResponse newLesson =
          new LessonResponse(
              oldLesson.id(),
              freshClass.id(),
  ...
  public TimetableFullResponse findById(UUID id, Integer orgId) {
    ...
    Integer maxVersion = timetableDataRepository.findMaxVersion(id);
    List<TimetableDataResponse> data =
        timetableDataRepository.findAllByTimetableIdAndVersion(id, maxVersion).stream()
    ...
    // Agar group repository bo'lmasa, hozircha bo'sh ro'yxat qaytaramiz
    // (GroupResponse ClassResponse ichida bor).
    List<GroupResponse> groups =
        new ArrayList<>(); // TODO: Load groups if needed separate from class
