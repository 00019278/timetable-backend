...
public class SolverDataPreparer {
  ...
  public List<OrTLesson> prepareLessons(
    ...
    for (LessonResponse raw : rawLessons) {
      ...
      // lessonCount - haftalik necha soat dars borligini bildiradi
      // Har bir soat uchun alohida OrTLesson yaratish kerak!
      int lessonCount = raw.lessonCount() != null ? raw.lessonCount() : 1;

      // 1-holat: Dars guruhlarga bo'lingan (Split Lesson)
      if (raw.groupDetails() != null && !raw.groupDetails().isEmpty()) {

        // Har bir lessonCount uchun alohida dars yaratamiz
        for (int lessonInstance = 0; lessonInstance < lessonCount; lessonInstance++) {
          // Har bir instance uchun YANGI sharedSyncId - bu instance ning guruhlari bir
          // vaqtda bo'ladi
          String sharedSyncId = UUID.randomUUID().toString();

          for (GroupLessonDetailResponse detail : raw.groupDetails()) {
            TeacherResponse dTeacher =
                detail.teacherId() != null ? teacherMap.get(detail.teacherId()) : null;
            SubjectResponse dSubject =
                detail.subjectId() != null ? subjectMap.get(detail.subjectId()) : null;
        ...
        for (int lessonInstance = 0; lessonInstance < lessonCount; lessonInstance++) {
          OrTLesson simpleLesson =
              new OrTLesson(
                  ...
                  null, // SyncId yo'q - ular bir vaqtda bo'lmasligi kerak!
                  1, // <-- Har bir instance 1 soat!
                  raw.dayOfWeek(),
                  raw.hour(),
                  raw.period(),
